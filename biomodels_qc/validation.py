    ...
    with open(filename, 'rb') as file:
        for line in file:
            if b'#Something went wrong' in line or b'We are sorry' in line:
                return [[f'PDF image {filename} was not successfully generated by the SBFC.']], []
...
def validate_entry(dirname, file_extensions=None, filenames=None, simulators=None):
    ...
    Args:
        ...
        simulators (:obj:`list` of :obj:`dict`, optional): specifications of simulators to use to check that SED-ML files can be executed.
            Each dictionary should have the schema ``https://api.biosimulators.org/openapi.json#/components/schemas/Simulator``,
            or two keys ``id`` and ``version``.
